...
skip_words = ['a', 'about', 'all', 'an', 'another', 'any', 'around', 'at',
              ...
              'of', 'off', 'oh', 'on', 'please', 'pls', 'small', 'some', 'soon',
...
def filter_words(words, skip_words):
    ...
    c=0
    while c<len(words):
        if words[c] in skip_words or words[c]=="":
...
def normalise_input(user_input):
    ...
    """
    no_punct = remove_punct(user_input).lower() #lowercase it all
    splat = no_punct.split(" ")     #split into a list around the spaces
    normalised=filter_words(splat,skip_words)       #remove the unnecessary words and the empty ones
    return normalised
